...
if (!todos) {
	todos = [
		{content: "Shopping", status: true, date: [28, 8, 2021]},
		{content: "Watch videos", status: false, date: [28, 8, 2021]},
		{content: "Like videos", status: true, date: [28, 8, 2021]},
	]
	...
	if (action.dataset.action == "add") {
		action.addEventListener("click", e => {
			formWrapper.innerHTML = `
				<form id="add">
					<input class="form-control" name="add" placeholder="press enter for submit..">
					<input type="date" class="form-control text-center" id="addtime" name="addtime">
				</form>
			`
			createTodos(todos)
			...
			add.addEventListener("submit", e => {
				...
				if (addtime.value) {
					day = addtime.value.substring(8, 10)
					month = addtime.value.substring(5, 7)
					year = addtime.value.substring(0, 4)
					if (add.add.value) {
						todos.push(
